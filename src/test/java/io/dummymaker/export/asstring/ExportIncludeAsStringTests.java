...
public class ExportIncludeAsStringTests extends Assertions {

    private final GenFactory factory = GenFactory.build();

    public static Stream<Arguments> data() {
        return Stream.of(
                Arguments.of(JsonExporter.builder().includeFields("num").build(), new JsonValidatorChecker(), 2),
                Arguments.of(CsvExporter.builder().includeFields("num").withHeader(false).build(), new CsvValidatorChecker(), 2),
                Arguments.of(SqlExporter.builder().includeFields("num").build(), new SqlValidatorChecker(), 8),
                Arguments.of(XmlExporter.builder().includeFields("num").build(), new XmlValidatorChecker(), 8));
    }
